...
public class ClientComponent implements Runnable
{
    ...
    public ClientComponent()
    {
        ...
        catch(UnknownHostException e){System.out.println("创建multicastIA异常");}
        initCommonFrame();
    }
    /**
     * 此构造方法需要设定组播IP地址、端口号、发送图片最大序号、发送分的块数
     */
    public ClientComponent(String multicastIP,int port,int seqmax,int blocknumber)
    {
        try
        {
...
//                            + "/" + packet.getAddress().getHostName() + "</html>";
                ...
                catch (Exception e) { System.err.println("客户端线程run出现错误"); }
            }//end while
...
//    {
//        ClientComponent client = new ClientComponent("224.0.0.2",9997,10,4);
